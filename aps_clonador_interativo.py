...
def _detect_columns(ws, header_row: int) -> dict:
    ...
    return {
        ...
        "pontuacao": pick("Pontua̤̣o"),
        "classif": pick("Classifica̤̣o"),
        "prioridade": pick("Prioridade"),
        "pendencias": pick("Pend̻ncias"),
    }
...
class EditorPlanilhaApp(tk.Toplevel):
    def __init__(self, workbook_path, master=None):
        ...
        self.filter_micro_var = tk.StringVar()
        self.filter_field_var = tk.StringVar(value="Todos")
    ...
    def _build(self):
        ...
        tk.Label(filters, text="Campo:", bg="#EAF2F8").pack(side="left", padx=(4, 0))
        cb_field = ttk.Combobox(
            filters,
            textvariable=self.filter_field_var,
            values=("Todos", "Nome", "Bairro", "Endereco", "Telefone"),
            state="readonly",
            width=11,
        )
        cb_field.pack(side="left", padx=(6, 6))
        cb_field.bind("<<ComboboxSelected>>", lambda _e: self._apply_filter())

        tk.Label(filters, text="Valor:", bg="#EAF2F8").pack(side="left")
        ent_micro = tk.Entry(filters, textvariable=self.filter_micro_var, width=12)
        ent_micro.pack(side="left", padx=(6, 8))
        ent_micro.bind("<KeyRelease>", self._schedule_filter)
        ...
        self.lbl_nome = tk.Label(panel, text="Paciente: -", bg="#FFFFFF", anchor="w", font=("Segoe UI", 12, "bold"))
        self.lbl_nome.grid(row=0, column=0, columnspan=2, sticky="ew", padx=12, pady=(12, 8))
        self.lbl_meta = tk.Label(panel, text="", bg="#FFFFFF", anchor="w", justify="left")
        self.lbl_meta.grid(row=1, column=0, columnspan=2, sticky="ew", padx=12, pady=(0, 12))
        self.frm_criterios = tk.Frame(panel, bg="#FFFFFF")
        self.frm_criterios.grid(row=2, column=0, columnspan=2, sticky="nsew", padx=12)

        mass = tk.LabelFrame(panel, text="Alteracao em massa", bg="#FFFFFF", fg="#1F4E79")
        ...
        tk.Button(actions, text="Aplicar em massa", command=self.apply_mass_change).pack(side="left", padx=8)
        tk.Button(actions, text="Salvar", command=self.flush_buffer, bg="#1F4E79", fg="white", font=("Segoe UI", 10, "bold")).pack(side="left", padx=8)
        tk.Button(actions, text="Desfazer", command=self.undo_last_change).pack(side="left")
        tk.Button(actions, text="Historico de mudancas", command=self.open_change_history).pack(side="left", padx=8)
        tk.Button(actions, text="Abrir planilha", command=self._open_workbook).pack(side="left", padx=8)
        tk.Button(actions, text="Recarregar", command=lambda: self.reload_data(async_mode=True)).pack(side="left")

        tk.Label(panel, textvariable=self.buffer_var, bg="#FFFFFF", fg="#1F4E79", anchor="w").grid(row=5, column=0, columnspan=2, sticky="ew", padx=12, pady=(0, 12))
        chart_box = tk.LabelFrame(panel, text="Grafico ao vivo (clique para filtrar prioridade)", bg="#FFFFFF", fg="#1F4E79")
    ...
    def _refresh_cards(self):
        ...
        concluidos = sum(1 for p in self.records if self._norm_prio(p["prio"]) == "CONCLUIDO")
    ...
    def _build_records_view(self):
        ...
        merged.sort(key=lambda x: (x["pts"], str(x["nome"]).lower()))
        self.records = merged
    ...
    def reload_data(self, async_mode: bool = False):
        ...
        def work():
            ...
            wb.close()
            return patients, cols["criterios"], _weights(cols["criterios"])

        def done_ok(result):
            patients, criterios, pesos = result
    ...
    def _apply_filter(self):
        ...
        micro = self.filter_micro_var.get().strip().lower()
        filter_field = self.filter_field_var.get().strip()
        ...
        for p in self.records:
            ...
            endereco_val = str(p.get("endereco", "")).lower()
            ...
            if term and term not in nome and term not in bairro_val and term not in endereco_val:
                continue
            if micro:
                if filter_field == "Nome" and micro not in nome:
                    continue
                if filter_field == "Bairro" and micro not in bairro_val:
                    continue
                if filter_field == "Endereco" and micro not in endereco_val:
                    continue
                if filter_field == "Telefone" and micro not in tel_val:
                    continue
                if filter_field == "Todos" and micro not in nome and micro not in bairro_val and micro not in endereco_val and micro not in tel_val:
                    continue
            ...
            self.tree.insert(
                ...
                "end",
                iid=str(p["row"]),
                values=(prio_norm, p["nome"], p.get("bairro", ""), p["pts"]),
                tags=tuple(tags),
            )
        self._refresh_cards()
    ...
    def _stage_row_changes(self, row: int, values_by_title: dict[str, str], source: str):
    ...
    def apply_mass_change(self):
        ...
        self._push_undo()
        for row in rows:
            self._stage_row_changes(row, {criterio: value}, source="mass")
        self._append_history({
            "action": "mass_stage",
            "criterio": criterio,
            "value": value,
    ...
    def flush_buffer(self):
        ...
        def worker():
            backup = self.workbook_path.with_name(f"{self.workbook_path.stem}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}{self.workbook_path.suffix}")
            backup_ok = None
            try:
            ...
            return applied_cells, backup_ok

        def on_ok(result):
            applied_cells, backup_ok = result
            ...
            self._set_busy(False)
            self.status_var.set(f"Alteracoes salvas com sucesso ({len(pending)} linhas, {applied_cells} campos).")
            self._build_records_view()
            self._apply_filter()
...
def launch_editor(master=None, workbook_path=None):
    if not workbook_path:
        workbook_path = filedialog.askopenfilename(title="Selecione a planilha APS", filetypes=[("Excel", "*.xlsx")])
        if not workbook_path:
            return None
    return EditorPlanilhaApp(workbook_path, master=master)
