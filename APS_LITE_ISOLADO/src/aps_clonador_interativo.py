...
class EditorPlanilhaApp(tk.Toplevel):
    ...
    def _discover_general_source_files(self) -> list[Path]:
        ...
        if folder.exists():
            for p in folder.glob("*.xlsx"):
                n = p.name.lower()
                if not re.search(r"c\d+", p.stem, re.I):
                    continue
    ...
    def _on_select(self, _evt=None):
        ...
        if self.unified_mode:
            ...
            if not ind_list:
                ind_list = sorted({(re.search(r"(C\d+)", f.stem, re.I).group(1).upper()) for f in self.general_source_files if re.search(r"(C\d+)", f.stem, re.I)})
    ...
    def open_indicator_source(self):
        ...
        if current_sources:
            for p in current_sources:
                mm = re.search(r"(C\d+)", p.stem, re.I)
                if mm and mm.group(1).upper() == code:
                    candidates.append(p)
        else:
            folder = Path(self.source_dir_var.get().strip() or self.workbook_path.parent)
            if not folder.exists():
                ...
                return
            candidates = [
                p for p in folder.rglob(f"{code}*.xlsx")
